 ...
 * 编辑文件夹名称，标签名称等
 */
public class TNTextEditAct extends TNActBase implements OnClickListener,OnKeyListener{
	...
	private void save(){
		...
		if( mTextType.equals("cat_add")) {
			TNAction.runActionAsync(TNActionType.FolderAdd, 
					mParentId, text);
		} else if( mTextType.equals("cat_rename")) {
			if( !text.equals(mOriginalText)) {
				TNAction.runActionAsync(TNActionType.FolderEdit, 
						mParentId, text);
			} else {
				finish();
			}
		} else if( mTextType.equals("tag_add")) {
			TNAction.runActionAsync(TNActionType.TagAdd, text);
		} else if( mTextType.equals("tag_rename")) {
			if( !text.equals(mOriginalText)){
				TNAction.runActionAsync(TNActionType.TagRename, mParentId, text);
			} else {
				finish();
			}
		}
	...
	public void respondFolderAdd(TNAction aAction) {
	...
	public void respondFolderEdit(TNAction aAction) {
		if (!TNHandleError.handleResult(this, aAction)) {
	...
	public void respondTagAdd(TNAction aAction) {
		if (!TNHandleError.handleResult(this, aAction)) {
			TNUtilsUi.showToast("保存成功！");
			finish();
		}
	...
	public void respondTagRename(TNAction aAction) {
		if (!TNHandleError.handleResult(this, aAction)) {
			TNUtilsUi.showToast("修改成功！");
			finish();
		}
