...
//				} else 
				{ // We haven't picked up a scroll event yet; check for one.
					...
					mLastMotionY = y;
					mLastMotionX = x;
//					Log.e("info", "Y: " + mLastMotionY + "-->" + y + "=" + yDiff);
//					Log.e("info", "X: " + mLastMotionX + "-->" + x + "=" + xDiff);
					if (yMoved && yDiff > xDiff) {
						mTouchState = TOUCH_STATE_VERTICAL_SCROLLING;
//						mLastMotionY = y;
						intercept = false;
					}else if (xMoved && xDiff > yDiff) {
...
//		Log.e("info", "action=" + action + " intercept=" + intercept + " touchState=" + mTouchState);
		return intercept;
	}

	@Override
	public boolean onTouchEvent(final MotionEvent ev) {
//		Log.e("onTouchEvent", "" + isClickable() + isEnabled() + this);
		if( !isEnabled())
			return true;
